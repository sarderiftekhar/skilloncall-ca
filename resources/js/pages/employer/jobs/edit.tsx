...
interface Job {
    ...
    province: string;
    city: string;
    global_province_id: number | null;
    global_city_id: number | null;
    job_type: string;
...
interface EditJobPageProps {
    ...
    globalSkills?: { id: number; name: string; category: string }[];
}
...
export default function EditJobPage({ job, categories, globalSkills = [] }: EditJobPageProps) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        required_skills: job.required_skills ? job.required_skills.map((s) => s.name) : [],
        province: job.province || '',
        city: job.city || '',
        global_province_id: job.global_province_id || null,
        global_city_id: job.global_city_id || null,
    ...
    }, [data.province, previousProvince, setData]);
    ...
    const skillsByCategory = globalSkills.reduce((acc: Record<string, { id: number; name: string; category: string }[]>, skill: { id: number; name: string; category: string }) => {
    ...
    const filteredSkillsByCategory = filteredSkills.reduce((acc: Record<string, { id: number; name: string; category: string }[]>, skill: { id: number; name: string; category: string }) => {
        if (!acc[skill.category]) {
            acc[skill.category] = [];
        }
        acc[skill.category].push(skill);
                ...
                <div className="mx-auto max-w-3xl">
                    ...
                    <div className="mb-6">
                        <h1 className="text-2xl md:text-3xl font-bold leading-tight job-title mb-2">
                            {t('jobs.edit.title', 'Edit Job')}
                        </h1>
                        <p className="text-lg leading-relaxed text-gray-600">
                            {t('jobs.edit.subtitle', 'Update the details of your job listing')}
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit}>
                        <Card className="border" style={{ borderColor: '#10B3D6', borderWidth: '0.05px' }}>
                            <CardHeader className="pb-3">
                                <CardTitle className="text-lg job-title">
                                    {t('jobs.edit.job_details', 'Job Details')}
                                </CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-4">
                                ...
                                <div id="title">
                                    <Label htmlFor="title" className="text-sm font-medium">
                                        {t('jobs.edit.title_label', 'Job Title')} <span className="text-red-500">*</span>
                                    </Label>
                                    <Input
                                        id="title"
                                        type="text"
                                        ...
                                        placeholder={t('jobs.edit.title_placeholder', 'e.g., Restaurant Server Needed')}
                                ...
                                <div id="description">
                                    <Label htmlFor="description" className="text-sm font-medium">
                                        {t('jobs.edit.description_label', 'Job Description')} <span className="text-red-500">*</span>
                                    </Label>
                                    <Textarea
                                        id="description"
                                        value={data.description}
                                        onChange={(e) => setData('description', e.target.value)}
                                        placeholder={t('jobs.edit.description_placeholder', 'Describe the job requirements, responsibilities, and expectations')}
                                ...
                                <div id="category">
                                    <Label htmlFor="category" className="text-sm font-medium">
                                        {t('jobs.edit.category_label', 'Category')} <span className="text-red-500">*</span>
                                    </Label>
                                    <Select
                                        value={data.category}
                                    ...
                                    >
                                        <SelectTrigger className={`mt-1 cursor-pointer ${errors.category ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : ''}`}>
                                            <SelectValue placeholder={t('jobs.edit.category_placeholder', 'Select a category')} />
                                        </SelectTrigger>
                                        <SelectContent>
                                ...
                                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                    <div id="job_type">
                                        <Label htmlFor="job_type" className="text-sm font-medium">
                                            {t('jobs.edit.job_type_label', 'Job Type')} <span className="text-red-500">*</span>
                                        </Label>
                                        <Select
                                            value={data.job_type}
                                        ...
                                        >
                                            <SelectTrigger className={`mt-1 cursor-pointer ${errors.job_type ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : ''}`}>
                                                <SelectValue placeholder={t('jobs.edit.job_type_placeholder', 'Select job type')} />
                                            </SelectTrigger>
                                            <SelectContent>
                                                {JOB_TYPES.map((type) => (
                                    ...
                                    <div id="experience_level">
                                        <Label htmlFor="experience_level" className="text-sm font-medium">
                                            {t('jobs.edit.experience_label', 'Experience Level')} <span className="text-red-500">*</span>
                                        </Label>
                                        <Select
                                        ...
                                        >
                                            <SelectTrigger className={`mt-1 cursor-pointer ${errors.experience_level ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : ''}`}>
                                                <SelectValue placeholder={t('jobs.edit.experience_placeholder', 'Select experience level')} />
                                            </SelectTrigger>
                                            <SelectContent>
                                ...
                                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                    <div id="budget">
                                        <Label htmlFor="budget" className="text-sm font-medium">
                                            {t('jobs.edit.budget_label', 'Budget')} <span className="text-red-500">*</span>
                                        </Label>
                                        <Input
                                            id="budget"
                                            type="number"
                                            ...
                                            placeholder={t('jobs.edit.budget_placeholder', '0.00')}
                                    ...
                                    <div id="deadline">
                                        <Label htmlFor="deadline" className="text-sm font-medium">
                                            {t('jobs.edit.deadline_label', 'Deadline')} 
                                            <span className="text-gray-500 ml-1">({t('jobs.edit.optional', 'Optional')})</span>
                                        </Label>
                                        <Input
                                            id="deadline"
                                            type="date"
                                ...
                                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                    <div id="province">
                                        <Label htmlFor="province" className="text-sm font-medium">
                                            {t('jobs.edit.province_label', 'Province')} <span className="text-red-500">*</span>
                                        </Label>
                                        <Select
                                            value={data.province}
                                        ...
                                        >
                                            <SelectTrigger className={`mt-1 cursor-pointer ${errors.province ? 'border-red-500 focus:ring-red-500 focus:border-red-500' : ''}`}>
                                                <SelectValue placeholder={t('jobs.edit.province_placeholder', 'Select province')} />
                                            </SelectTrigger>
                                            <SelectContent>
                                    ...
                                    <div ref={cityInputRef} id="city" className="relative">
                                        <Label htmlFor="city" className="text-sm font-medium">
                                            {t('jobs.edit.city_label', 'City')} <span className="text-red-500">*</span>
                                        </Label>
                                        <div className="relative mt-1">
                                            <Input
                                                ...
                                                placeholder={
                                                    data.province 
                                                        ? t('jobs.edit.city_placeholder', 'Type to search cities...') 
                                                        : t('jobs.edit.city_placeholder_disabled', 'Select a province first')
                                                }
                                            ...
                                            {showCitySuggestions && loadingCities && (
                                                <div className="absolute z-50 mt-1 w-full rounded-md border border-gray-200 bg-white p-3 text-sm text-gray-500 shadow-lg">
                                                    {t('jobs.edit.loading_cities', 'Loading cities...')}
                                                </div>
                                            )}
                                            ...
                                            {showCitySuggestions && !loadingCities && citySearch && cities.length === 0 && (
                                                <div className="absolute z-50 mt-1 w-full rounded-md border border-gray-200 bg-white p-3 text-sm text-gray-500 shadow-lg">
                                                    {t('jobs.edit.no_cities', 'No cities found matching')}{' '}"{citySearch}"
                                                </div>
                                            )}
                                        </div>
                                        {!data.province && (
                                ...
                                <div id="required_skills">
                                    <Label className="text-sm font-medium">
                                        {t('jobs.edit.skills_label', 'Required Skills')} <span className="text-red-500">*</span>
                                    </Label>
                                        ...
                                        <div className="flex-1 relative" ref={skillInputRef}>
                                            <div className="relative">
                                                <Input
                                                    ...
                                                    placeholder={t('jobs.edit.skills_placeholder', 'Type to search skills or select from list...')}
                                                    className="w-full"
                                                />
                                                        ...
                                                        {skillSearch.length === 0 ? (
                                                            ...
                                                            Object.entries(skillsByCategory).map(([category, skills]) => (
                                                                <div key={category}>
                                                                    ...
                                                                    {(skills as { id: number; name: string; category: string }[]).map((skill) => {
                                                    ...
                                                    <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg">
                                                        <div className="p-3">
                                                            <p className="text-sm text-gray-500 mb-2">
                                                                {t('jobs.edit.no_skills_found', 'No skills found matching')} "{skillSearch}"
                                                            </p>
                                                            <p className="text-xs text-gray-400">
                                                                {t('jobs.edit.add_custom_skill_hint', 'Press Enter or click Add to add as custom skill')}
                                                            </p>
                                                        </div>
                                                    </div>
                                                )}
                        ...
                        <div className="mt-6 flex justify-end gap-4">
                            ...
                            >
                                ...
                                {processing ? t('jobs.edit.updating', 'Updating...') : t('jobs.edit.update_job', 'Update Job')}
                            </Button>
                        </div>
                    </form>
                </div>
